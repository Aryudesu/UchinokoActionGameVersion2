...
public:
	...
	static bool TryFind1x2(const TileMap& Map, const TileCatalog& Catalog,
		int WorldX, int WorldY, Slope1x2& Result);
	static float SurfaceY(const Slope1x2& Slope, float WorldX);
	static bool TryCharacterY1x2(const TileMap& Map, const TileCatalog& Catalog,
		float WorldX, float ProbeY, float& CharacterY, Slope1x2* Found = nullptr);
	static bool FollowHorizontal1x2(const TileMap& Map, const TileCatalog& Catalog,
		float OldX, float NewX, float OldY, float& NewY,
		int VelocityX10, int& VelocityY10, bool WasGrounded, bool& Grounded);
	static bool ResolveHighSide1x2(const TileMap& Map, const TileCatalog& Catalog,
		float OldX, float& NewX, float Y, bool MovingRight, bool Grounded);
	static bool ResolveRising1x2(const TileMap& Map, const TileCatalog& Catalog,
		float X, float OldY, float& NewY);
	static bool ResolveFalling1x2(const TileMap& Map, const TileCatalog& Catalog,
		float X, float OldY, float& NewY);
};
